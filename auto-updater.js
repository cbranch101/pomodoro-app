...
function checkForMacUpdate() {
    return axios.get(updateURL).then(response => {
        ...
        return response.status === 200 && semver.gt(response.data.version, version)
    })
    // .catch(console.error)
}
...
export function checkForUpdate() {
    ...
    if (process.platform === "win32") {
        ...
        // a simple file server for mac updates,
        // not the HTTP 200 status = update, 204 = no update thing,
